...
Extracting in place fails due to permission errors:

```bash
tar -xvzf backup-ssh-identity-files.tgz
```

![Tar Permission Error](images/10.JPG)

The working directory is changed to `/tmp` and the archive is extracted from there:
...
![Tar Extract + id_rsa](images/11.JPG)

The key is **encrypted with a passphrase** (`Proc-Type: 4,ENCRYPTED`).

### Save and crack the key

The key is copied to the attacking machine and saved with an editor:

![Save id_rsa](images/12.JPG)
...
![SSH Passphrase prompt](images/13.JPG)
...
![SSH David + user.txt](images/15.JPG)
...
![journalctl command](images/17.JPG)

![journalctl output + shell escape](images/18.JPG)
