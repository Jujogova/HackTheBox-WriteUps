...
Al intentar extraer directamente en el directorio actual los permisos lo impiden:

```bash
tar -xvzf backup-ssh-identity-files.tgz
```

![Tar Error Permisos](images/10.JPG)

Se cambia a `/tmp` y se extrae desde allí:
...
Se copia la clave en la máquina atacante guardándola con el editor:

![Guardar id_rsa](images/12.JPG)
...
![SSH pide passphrase](images/13.JPG)
...
![journalctl comando](images/17.JPG)

![journalctl output + escape a shell](images/18.JPG)
